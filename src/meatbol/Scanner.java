...
public class Scanner {
    ...
    private int createOperand(String substring, int lineNum, int index)
    {
        ...
        // Otherwise, create the token and return the new column position after
        // the operand
    ...
    private int createStringToken(String substring, int lineNum, int index)
    {
        ...
        int i = 0; // loop counter
        char[] lineData = substring.toCharArray(); // converts String to char[]
        char tab = 0x09; // ANSI value for tab
        char newLine = 0x0a; // ANSI value for new line
        ...
        for (i = 1; i < lineData.length; i++)
        {
            ...
            {
                currentToken = setToken(substring.substring(1, i - trimEscapes)
                        , Classif.OPERAND, SubClassif.STRING, lineNum, index);
                break;
            }

        }
        // if no match by end of the line, throw exception and exit
        if (i == lineData.length)
            throw new IllegalArgumentException(
                    "Syntax Error: missing closing quote at Line " + (lineNum + 1) + " - " + substring);
        // Otherwise, create the token and return the new column position
        currentToken = setToken(substring.substring(1, i - trimEscapes)
                , Classif.OPERAND, SubClassif.STRING, lineNum, index);
        return index + i;
    }
